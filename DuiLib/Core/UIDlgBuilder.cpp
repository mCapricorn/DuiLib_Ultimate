...
namespace DuiLib
{
            ...
            for (CMarkupNode node = root.GetChild(); node.IsValid(); node = node.GetSibling())
            {
                ...
                else if (_tcsicmp(pstrClass, _T("Font")) == 0)
                {
                    ...
                    if (id >= 0)
                    {
                        ...
                        if (defaultfont)
                            pManager->SetDefaultFont(pFontName, size, bold, underline, italic, strikeout, shared);
                    }
                }
    ...
    CControlUI *CDialogBuilder::_Parse(CMarkupNode *pRoot, CControlUI *pParent, CPaintManagerUI *pManager)
    {
        ...
        for (CMarkupNode node = pRoot->GetChild(); node.IsValid(); node = node.GetSibling())
        {
            ...
            if (_tcsicmp(pstrClass, _T("Image")) == 0 || _tcsicmp(pstrClass, _T("Font")) == 0 || _tcsicmp(pstrClass, _T("Default")) == 0 || _tcsicmp(pstrClass, _T("Style")) == 0)
                continue;
