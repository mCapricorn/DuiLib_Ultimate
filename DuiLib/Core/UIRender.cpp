...
namespace DuiLib
{
    ...
    void CRenderEngine::DrawRect(HDC hDC, const RECT &rc, int nSize, DWORD dwPenColor, int nStyle /*= PS_SOLID*/)
    {
        ...
        graphics.DrawRectangle(&pen,
                               static_cast<INT>(rc.left),
                               static_cast<INT>(rc.top),
                               static_cast<INT>(rc.right - rc.left - 1),
                               static_cast<INT>(rc.bottom - rc.top - 1));
#endif
    }
                ...
                case _T('f'): // Font
                {
                    ...
                    else
                    {
                        ...
                        if (sFontAttr.Find(_T("italic")) >= 0)
                            bItalic = true;
                        if (sFontAttr.Find(_T("strikeout")) >= 0)
                            bStrikeout = true;
                ...
                case _T('i'): // Italic or Image
                {
                    ...
                    { // Italic
                        ...
                        if (pFontInfo->bItalic == false)
                        {
                            ...
                            if (hFont == NULL)
                                hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName, pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, pFontInfo->bStrikeout, true);
                            pFontInfo = pManager->GetFontInfo(hFont);
                            aFontArray.Add(pFontInfo);
                            pTm = &pFontInfo->tm;
                            ::SelectObject(hDC, pFontInfo->hFont);
