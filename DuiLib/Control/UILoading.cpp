...
CLoadingUI::CLoadingUI()
    : m_bStop(true), m_nTime(100), m_nNumber(0), m_Angles(nullptr), m_Colors(nullptr), m_NumberOfSpoke(10), m_SpokeThickness(4), m_OuterCircleRadius(10), m_InnerCircleRadius(8)
{
}
...
void CLoadingUI::PaintBkImage(HDC hDC)
{
    ...
    if (m_NumberOfSpoke > 0)
    {
        ...
        // ��ô��ڵ�Graphics����
        Graphics gh(hDC);
        // blit cached image to target DC
        gh.DrawImage(img, static_cast<INT>(m_rcItem.left), static_cast<INT>(m_rcItem.top));
        delete img;
    }
}
...
Color *CLoadingUI::GenerateColorsPallet(Color _objColor, bool _blnShadeColor, int _intNbSpoke)
{
    ...
    BYTE bytIncrement = static_cast<BYTE>(255 / m_NumberOfSpoke);
